...
class MTPManager:
    ...
    def copy(self, src, dest, overwrite: Optional[bool] = False) -> None:
        ...
        self._kill_process(self.process_name)

        self._manage_storage("mount")
        overwrite_flag = "/Y"
        ...
        unmount = self._manage_storage("unmount")
        if unmount.returncode == 0:
            self._kill_process(self.process_name)

    def _manage_storage(self, operation: str) -> subprocess.CompletedProcess:
        """
        ...
        try:
            ...
            mount_operation = self._run(cmd, check=True)
    ...
    def _kill_process(self, process_name: str) -> None:
        ...
        process_check = self._run(cmd, capture_output=True, text=True)
